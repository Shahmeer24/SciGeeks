...
const Navbar = () => {
  ...
  const getLinkClass = (path) => {
    ...
    const baseClasses =
      "relative font-display text-base font-medium transition-colors duration-300 px-3 py-1.5 rounded-full";
    const activeClasses =
      "bg-slate-100 dark:bg-gray-800 text-slate-900 dark:text-white";
    const inactiveClasses =
      "text-slate-600 dark:text-slate-300 before:absolute before:left-0 before:-bottom-1 before:h-0.5 before:w-full before:origin-right before:scale-x-0 before:bg-slate-600 before:transition-transform before:duration-300 dark:before:bg-slate-400 hover:before:origin-left hover:before:scale-x-100";
  ...
  const getMobileLinkClass = (path) => {
    const isActive = location.pathname === path;
    return `block px-4 py-2 rounded-md text-lg transition-colors ${
      isActive
                ...
                <div className="bg-white dark:bg-gray-800 rounded-md shadow-lg p-2">
                  {dropdownLinks.map((link) => (
                    <Link
                      key={link.path}
                      to={link.path}
                      className="block px-4 py-2 text-base text-slate-700 dark:text-slate-300 rounded-md hover:bg-slate-100 dark:hover:bg-gray-700"
                    >
                      {link.label}
                    </Link>
                  ))}
          ...
          <div className="flex items-center space-x-4">
            {user && !user.isAnonymous ? (
              <div className="hidden md:flex items-center gap-4">
                <div
                  className="group relative w-10 h-10 bg-slate-600 dark:bg-slate-400 rounded-full flex items-center justify-center text-white font-bold text-base"
                ...
                >
                  ...
                  {/*Tooltip */}
                  <span className="absolute top-12 left-1/2 -translate-x-1/2 scale-0 transition-transform group-hover:scale-100 px-3 py-1.5 bg-gray-800 text-white text-sm rounded-md shadow-lg whitespace-nowrap">
                    {user.email}
                  </span>
                </div>
                <button
              ...
              <Link
                to="/auth"
                className="hidden md:inline-block bg-slate-600 text-white px-5 py-2 rounded-full text-base font-semibold hover:bg-slate-700 transition-colors"
              >
                Register / Login
              </Link>
            )}
            ...
            <button
              onClick={toggleTheme}
              className="hidden md:inline-flex p-2 rounded-full hover:bg-slate-200 dark:hover:bg-gray-700 transition-colors"
            >
              {theme === "light" ? (
          ...
          <nav className="flex flex-col space-y-2 p-4">
            ...
            {user && !user.isAnonymous ? (
              <button
                ...
                className={`${getMobileLinkClass(null)} w-full text-left`}
              >
                Logout
              </button>
            ) : (
              <Link
                to="/auth"
                onClick={() => setIsMenuOpen(false)}
                className={getMobileLinkClass("/auth")}
              >
                Register / Login
              </Link>
            )}
            <div className="pt-4 flex justify-center">
              <button
                onClick={toggleTheme}
                className="flex items-center gap-3 p-3 rounded-full hover:bg-slate-200 dark:hover:bg-gray-700 transition-colors text-slate-600 dark:text-slate-300"
              >
                {theme === "light" ? (
